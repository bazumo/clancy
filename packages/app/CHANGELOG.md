# clancy-proxy

## 0.0.6

### Patch Changes

- add include/exclude hosts functionality
- Updated dependencies [adfef35]
- Updated dependencies
  - @clancyapp/backend@0.0.6

## 0.0.5

### Patch Changes

- 03fa9a6: Add token counter, improve UI
- 3e18a41: make utls esm
- 3e18a41: Improve LLM response view
- 1313711: Split up backend from app
- 54da17b: Restructure to monorepo, move utls to separate package
- Release new minor version
- Updated dependencies [3e18a41]
- Updated dependencies [3e18a41]
- Updated dependencies [1313711]
- Updated dependencies
  - @clancyapp/backend@0.0.5

## 0.0.5-beta.5

### Patch Changes

- Add token counter, improve UI
  - @clancyapp/backend@0.0.5-beta.5

## 0.0.5-beta.4

### Patch Changes

- @clancyapp/backend@0.0.5-beta.4

## 0.0.5-beta.3

### Patch Changes

- make utls esm
- Updated dependencies
  - @clancyapp/backend@0.0.5-beta.3

## 0.0.5-beta.2

### Patch Changes

- Improve LLM response view
- Updated dependencies
  - @clancyapp/backend@0.0.5-beta.2

## 0.0.5-beta.1

### Patch Changes

- Split up backend from app
- Updated dependencies
  - @clancyapp/backend@0.0.5-beta.1
