...
This section provides comprehensive usage examples for the Claude Messages API.
...
console.log(message.usage); // { input_tokens: X, output_tokens: Y }
```
...
const message = await client.messages.create({
  model: 'claude-sonnet-4-5-20250929',
  max_tokens: 1024,
  system: 'You are a helpful assistant that responds in a friendly, casual tone.',
  messages: [
    { role: 'user', content: 'What is TypeScript?' }
  ],
});

// System prompt can also be an array of content blocks (for caching)
const messageWithCachedSystem = await client.messages.create({
  model: 'claude-sonnet-4-5-20250929',
  max_tokens: 1024,
  system: [
    {
...
When streaming, you'll receive the following event types in order:

```typescript
// 1. message_start - Contains initial message with empty content
{ type: 'message_start', message: { id, model, role, content: [], usage } }

// 2. content_block_start - For each content block
{ type: 'content_block_start', index: 0, content_block: { type: 'text', text: '' } }

// 3. content_block_delta - Multiple deltas per block
{ type: 'content_block_delta', index: 0, delta: { type: 'text_delta', text: '...' } }

// 4. content_block_stop - When block is complete
{ type: 'content_block_stop', index: 0 }

// 5. message_delta - Final message metadata
{ type: 'message_delta', delta: { stop_reason: 'end_turn' }, usage: { output_tokens } }
...
if (message.stop_reason === 'tool_use') {
  ...
  console.log(toolUse.input);  // { location: 'San Francisco, CA' }
  console.log(toolUse.id);     // 'toolu_...'
}
```

## Tool Use with Tool Results
  ...
  tools: [
    {
      ...
      name: 'web_search',
      max_uses: 5,
      allowed_domains: ['wikipedia.org', 'github.com'], // optional
...
### Computer Use Tools (Bash, Text Editor)
...
for await (const event of stream) {
  if (event.type === 'content_block_delta') {
    if (event.delta.type === 'thinking_delta') {
      ...
      // Signature for thinking verification
    }
  }
}
```
...
const message = await client.messages.create({
  ...
  system: [
    {
      type: 'text',
      text: 'Very long system prompt that should be cached...',
      cache_control: { type: 'ephemeral', ttl: '1h' }
    }
  ],
  messages: [
...
console.log(message.usage.cache_creation_input_tokens); // Tokens used to create cache
```
...
const message = await client.messages.create({
  ...
  // Sampling parameters
  temperature: 0.7,      // 0-1, higher = more creative (default: 1)
  top_p: 0.9,            // Nucleus sampling (default: use temperature)
...
console.log(batch.processing_status); // 'in_progress' | 'ended'
...
if (status.processing_status === 'ended') {
  ...
  for await (const result of results) {
    if (result.result.type === 'succeeded') {
      ...
      console.log(result.custom_id, 'failed:', result.result.error);
    }
  }
}
